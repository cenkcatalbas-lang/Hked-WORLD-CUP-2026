...
@st.cache_data
def load_data():
    # Sizin yüklediğiniz HKED.xlsx dosyasını okuyoruz
    df = pd.read_excel("HKED.xlsx")
    return df

df = load_data()

# Maç Sonuçlarını Girme Alanı (Admin Paneli gibi)
...
# Örnek olarak ilk 2 maçı varsayılan olarak girilmiş yapalım
default_results = {0: "1", 1: "1"} 

for index, row in df.iterrows():
    match_label = f"{row['TAKIM - 1']} - {row['TAKIM - 2']} ({row['TARİH'].strftime('%d.%m.%Y')})"
...
for idx, res in results.items():
    row = df.iloc[idx]
    # Seçilen sonuca göre katsayıyı al (1, 0 veya 2 sütunundan)
    odd = float(row[res]) 
    
    for p in participants:
        p_clean = p.strip()
        # Eğer katılımcının tahmini, maç sonucu ile aynıysa katsayıyı ekle
        if int(row[p]) == res:
            scores[p_clean] += odd
